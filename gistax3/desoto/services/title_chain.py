...
def extract_text_from_pdf(pdf_path: str) -> str:
    """Extract text from PDF using pdfplumber first, then fall back to PyPDF2."""
    text = ""
...
def extract_table_entries_from_pdf(pdf_path: str) -> List[ChainEntry]:
    """Extract chain entries from well-formed PDF tables using pdfplumber only (no heuristics).

    This reads table cells, identifies the header row to map column indices, and parses each
    subsequent row into a ChainEntry. If no tables or entries are found, returns an empty list.
    """
...
def process_title_document(file_path: str, output_path: str = None, template_path: str = None) -> tuple[bool, str, List[ChainEntry], List[ChainEntry]]:
    ...
    try:
        ...
        # Parse the chain entries using table cells first (no heuristics), fallback to text
        entries = []
        try:
            if file_ext == '.pdf':
                entries = extract_table_entries_from_pdf(file_path)
        except Exception:
            entries = []
        if not entries:
