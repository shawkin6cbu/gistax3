"""
Service to split comprehensive title search PDFs into chain and tax components.
"""
import re
import io
import PyPDF2
...
def identify_page_type(text: str) -> str:
    ...
    # Chain of Title indicators
    chain_indicators = [
        'CHAIN OF TITLE',
        'FILED GRANTOR GRANTEE INSTRUMENT',
        'GRANTOR GRANTEE INSTRUMENT BOOK-PAGE',
        'WARRANTY DEED',
        'DEED OF TRUST',
        'TRUSTEE\'S DEED'
    ]
    ...
    # Count indicators
    chain_score = sum(1 for indicator in chain_indicators if indicator in text_upper)
    tax_score = sum(1 for indicator in tax_indicators if indicator in text_upper)
...
def extract_pages_by_type(pdf_path: str) -> Tuple[bytes, bytes, str]:
    ...
    try:
        # First pass: identify page types using pdfplumber for better text extraction
        page_types = []
        ...
        # If no pages identified, look for common patterns in page sequences
        if not chain_pages and not tax_pages:
            # Try to find pages with specific headers as starting points
            with open(pdf_path, 'rb') as file:
                pdf_reader = PyPDF2.PdfReader(file)
                for i in range(len(pdf_reader.pages)):
                    page = pdf_reader.pages[i]
                    ...
                    if 'CHAIN OF TITLE' in text.upper() and 'File No.' in text:
                        ...
                        if i + 1 < len(pdf_reader.pages):
                            next_text = pdf_reader.pages[i + 1].extract_text()
