...
class ProcessingTab(ttk.Frame):
    def __init__(self, parent, shared_data):
        ...
        ttk.Label(doc_drop_frame, textvariable=self.process_status_var, wraplength=350).grid(
    ...
    def _process_search_document_thread(self, file_path):
        try:
            ...
            def update_ui():
                ...
                if success and (chain_entries or tax_total):
                    messagebox.showinfo("Success", 
                        f"Document processed successfully!\n\n"
                        f"Chain: {len(chain_entries)} vesting deeds found\n"
                        f"Tax: ${tax_total or 'Not found'}\n"
                        f"Date Paid: {tax_date_paid or 'Not found'}")
                elif not success:
                    messagebox.showwarning("Processing Issue", msg)
    ...
    def _create_full_document(self, output_path):
        ...
        if doc_path and os.path.exists(doc_path) and not self.shared_data.get_data("title_chain_kept"):
            ...
            success, _, results = process_comprehensive_document(doc_path)
            if success:
                self.shared_data.set_data("title_chain_kept", results.get('chain_entries', []))
                self.shared_data.set_data("title_chain_all", results.get('all_entries', []))
